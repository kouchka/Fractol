...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/21 05:23:49 by allallem          #+#    #+#             */
/*   Updated: 2018/05/21 06:23:31 by allallem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void			ft_trace_x_bonus2(t_thread *p)
{
	int		i;
	double	zr;
	double	zi;
...
static void			ft_malloc_thread_bonus2(t_fractol *p)
{
	pthread_t	*tab;
	t_thread	**st;
	...
	|| !(st = malloc(sizeof(t_thread*) * THREAD)))
		exit(EXIT_SUCCESS);
	ft_go_on_thread(st, tab, p, &ft_work_b2);
	while (p->i < THREAD)
	{
